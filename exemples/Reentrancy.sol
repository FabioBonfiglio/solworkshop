...
********************************************************************************
Reentrancy.sol - Pseudo code source de démonstration de vulnérabilité reentrancy
...
-> NE COMPILE PAS forcément en l'état.
...
contract Reentrancy {
	...
	modifier minimumRequis(uint8 min) {
		require(val >= min, "Valeur trop faible!");
		_;
	}
	
	function set(uint8 newVal) public minimumRequis(0x80) {
		if (!locked) {
			val = newVal;
		}
		else {
			revert("Valeur verrouillée !")
		}
		locked = true;
	}
